...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   `omx team api send-message --input "{"team_name":"turn-the-updated-jarvis-fusion","from_worker":"worker-2","to_worker":"leader-fixed","body":"ACK: worker-2 initialized"}" --json`
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/dd/Projects/jarvis-fusion-system/.omx/state/team/turn-the-updated-jarvis-fusion/workers/worker-2/status.json`
...
1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"turn-the-updated-jarvis-fusion","worker":"worker-2"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"turn-the-updated-jarvis-fusion","worker":"worker-2","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"turn-the-updated-jarvis-fusion","from_worker":"worker-2","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
