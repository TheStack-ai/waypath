...
- **Task 1**: Implement: Turn the updated Jarvis Fusion planning docs into an implementation-r
  Description: Implement the core functionality for: Turn the updated Jarvis Fusion planning docs into an implementation-ready execution breakdown. Produce concrete next-step work packages, sequencing, milestone structure, and verification gates for the shared-backend + thin-host-shim architecture so the next implementation session can start immediately.
  Status: pending
  Role: planner
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   `omx team api send-message --input "{"team_name":"turn-the-updated-jarvis-fusion","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK: worker-1 initialized"}" --json`
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/dd/Projects/jarvis-fusion-system/.omx/state/team/turn-the-updated-jarvis-fusion/workers/worker-1/status.json`
...
1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"turn-the-updated-jarvis-fusion","worker":"worker-1"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"turn-the-updated-jarvis-fusion","worker":"worker-1","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"turn-the-updated-jarvis-fusion","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
