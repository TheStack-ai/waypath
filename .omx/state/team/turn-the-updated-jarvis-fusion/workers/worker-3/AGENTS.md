...
Supported workflow triggers include: `ralph`, `autopilot`, `ultrawork`, `ultraqa`, `cleanup`/`refactor`/`deslop`, `analyze`, `plan this`, `deep interview`, `ouroboros`, `ralplan`, `team`/`swarm`, `ecomode`, `cancel`, `tdd`, `fix build`, `code review`, `security review`, `web-clone`, `strategy-team`, `design-team`, `web-implementation-team`, `web-design-team`, `content-team`, `marketing-team`, `research-team`, and `ai-engineer-team`.
...
| "전략팀과 회의", "전략팀과 상의", "전략팀으로", "cto/cpo/cdo/coo 관점" | `$strategy-team` | Read `~/.codex/skills/strategy-team/SKILL.md`, run the OMX-native executive strategy team workflow |
| "디자인팀과 회의", "디자인팀과 상의", "디자인팀으로", "브랜딩 디자인" | `$design-team` | Read `~/.codex/skills/design-team/SKILL.md`, run the OMX-native general design team workflow |
| "웹구현팀", "웹 구현팀", "홈페이지 만들어줘", "랜딩페이지 만들어줘" | `$web-implementation-team` | Read `~/.codex/skills/web-implementation-team/SKILL.md`, run the OMX-native web implementation team workflow |
| "웹디자인팀", "웹사이트 디자인", "랜딩페이지 디자인" | `$web-design-team` | Read `~/.codex/skills/web-design-team/SKILL.md`, run the OMX-native web design team workflow |
| "콘텐츠팀", "카드뉴스 만들어줘", "ppt 만들어줘", "pdf 만들어줘", "엑셀 작업해줘" | `$content-team` | Read `~/.codex/skills/content-team/SKILL.md`, run the OMX-native content production team workflow |
| "마케팅팀", "캠페인 카피", "메시지 전략 짜줘", "gtm 방향" | `$marketing-team` | Read `~/.codex/skills/marketing-team/SKILL.md`, run the OMX-native marketing team workflow |
| "리서치팀", "브라우저로 찾아줘", "사이트를 열고 조작하면서 조사" | `$research-team` | Read `~/.codex/skills/research-team/SKILL.md`, run the OMX-native browser-first research team workflow |
| "ai-engineer팀", "ai 엔지니어팀", "모델/에이전트 구조 설계", "ai systems engineering" | `$ai-engineer-team` | Read `~/.codex/skills/ai-engineer-team/SKILL.md`, run the OMX-native AI engineer team workflow |
...
You are a team worker in team "turn-the-updated-jarvis-fusion". Your identity and assigned tasks are in your inbox file.
...
5. Read your task from <team_state_root>/team/turn-the-updated-jarvis-fusion/tasks/task-<id>.json (example: task-1.json)
6. Task id format:
...
12. Update your status: write {"state": "idle", "updated_at": "<current ISO timestamp>"} to <team_state_root>/team/turn-the-updated-jarvis-fusion/workers/{your-name}/status.json
13. Wait for new instructions (the lead will send them via your terminal)
14. Check your mailbox for messages at <team_state_root>/team/turn-the-updated-jarvis-fusion/mailbox/{your-name}.json
...
Example:
omx team api send-message --input "{"team_name":"turn-the-updated-jarvis-fusion","from_worker":"<your-worker-name>","to_worker":"leader-fixed","body":"ACK: <your-worker-name> initialized"}" --json
...
1. Read: `omx team api mailbox-list --input "{"team_name":"turn-the-updated-jarvis-fusion","worker":"<your-worker-name>"}" --json`
2. Mark delivered: `omx team api mailbox-mark-delivered --input "{"team_name":"turn-the-updated-jarvis-fusion","worker":"<your-worker-name>","message_id":"<MESSAGE_ID>"}" --json`
...
<!-- OMX:TEAM:ROLE:START -->
<team_worker_role>
You are operating as the **planner** role for this team run. Apply the following role-local guidance in addition to the team worker protocol.
