...
export function runImportLocalCliIntegrationTest(): void {
  ...
  try {
    ...
    assert(result.counts.entities >= 4, 'expected imported entities from both local adapters');
    assert(result.counts.relationships >= 2, 'expected imported relationships from both local adapters');
    assert(result.counts.decisions >= 1, 'expected imported jarvis decisions');
    assert(result.counts.preferences >= 1, 'expected imported jarvis preferences');
    assert(result.counts.promoted_memories >= 2, 'expected imported memories from both local adapters');
