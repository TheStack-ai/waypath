...
export function runSourceReaderBootstrapUnitTest(): void {
  const root = mkdtempSync(`${tmpdir()}/waypath-import-`);
  ...
  assertEqual(result.imported_relationships, 0);
  ...
  assertEqual(importResult.readers[0], 'demo-source');
  assertEqual(importResult.counts.entities, 1);
  assertEqual(importResult.counts.relationships, 0);
